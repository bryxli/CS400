...
/**
 * Graph that has a dijkstra algorithm implemented.
 * @param <T> Object data to be represented with this graph.
 */
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        if(vertices.containsKey(start)&&vertices.containsKey(end)) {
            Vertex startingNode = vertices.get(start);
            Vertex endingNode = vertices.get(end);
            if (startingNode.equals(endingNode)){
                return new Path(startingNode);
            }
            PriorityQueue<Path> queue = new PriorityQueue<>();
            LinkedList<Vertex> found = new LinkedList<>();
            found.add(startingNode);
            queue.add(new Path(startingNode));

            while(!queue.isEmpty()){
                Path path = queue.poll();
                LinkedList<Edge> edges = path.end.edgesLeaving;
                for (Edge edge : edges) {
                    if (!found.contains(edge.target)) {
                        found.add(edge.target);
                        Path updatedPath = new Path(path);
                        updatedPath.extend(edge);
                        if (updatedPath.end.equals(endingNode)) {
                            return updatedPath;
                        } else
